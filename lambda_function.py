...
def lambda_handler(event, context):
    ...
    decoded_key = urllib.parse.unquote_plus(text_file_key)
    audio_key = decoded_key.replace('.txt', '.mp3')

    try:
        # Retrieve text from the source S3 bucket
        logger.info(f"Retrieving text file from bucket: {source_bucket}, key: {decoded_key}")
        text_file = s3.get_object(Bucket=source_bucket, Key=decoded_key)
        ...
        logger.info(f"Text-to-Speech conversion completed successfully for file: {decoded_key}")
        ...
        logger.error(f"Error processing file {decoded_key} from bucket {source_bucket}: {str(e)}")
        return {
            'statusCode': 500,
